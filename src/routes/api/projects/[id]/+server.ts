...
export const GET: RequestHandler = async ({ params }) => {
  const result = await Effect.runPromise(Effect.either(getProject(params.id)))
...
export const PATCH: RequestHandler = async ({ params, request }) => {
  ...
  const result = await Effect.runPromise(Effect.either(updateProject(params.id, patch)))
  if (result._tag === 'Left') return errorResponse(result.left)
