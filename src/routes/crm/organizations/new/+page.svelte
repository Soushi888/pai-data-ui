<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    if (json.organization?.id) {
      goto(`/crm/organizations/${json.organization.id}`)
    } else {
      error = json.message ?? 'Failed to create organization'
      saving = false
    }
  }
</script>
...
<div class="p-6 max-w-xl">
  <div class="flex items-center gap-3 mb-6">
    ...
    <h1 class="text-xl font-semibold text-gray-100">New Organization</h1>
  </div>
  ...
  <form onsubmit={submit} onkeydown={(e) => { if (e.ctrlKey && e.key === 'Enter') e.currentTarget.requestSubmit() }} class="space-y-4">
    ...
    <div>
      ...
      <input bind:value={domain} placeholder="example.com" class="bg-gray-800 border border-gray-700 text-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 w-full" />
    </div>
    <div>
    ...
    {/if}
    <button type="submit" disabled={saving || !name} class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded transition-colors disabled:opacity-50">
